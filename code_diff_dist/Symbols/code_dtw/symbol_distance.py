...
def get_distance(size):
    ...
    for i in range(size):
        symbol_distance[symbols[i]+symbols[i]] = 0
...
# def euclidean_distance(symbol_size, str1, str2):
#     dist = 0
#     for i in range(len(str1)):
#         r = symbols.index(str1[i])
#         c = symbols.index(str2[i])
#         if abs(r-c)>1:
#             dist += cutlines[symbol_size][max(r,c)-1]-cutlines[symbol_size][min(r,c)]
#     return dist


def euclidean_distance(symbol_distance, str1, str2):
    dist = 0
    for i in range(len(str1)):
        dist += (symbol_distance[str1[i]+str2[i]])**2
    return np.sqrt(dist)


if __name__ == '__main__':
    print(get_distance(4))