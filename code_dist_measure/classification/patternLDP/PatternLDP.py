...
def perturb(para):
        ...
        file = open(str(index)+'data_e8.npy', 'wb')
        np.save(file, perturbed_data)
        file.close()
        perturbed_label = np.array(perturbed_label)
        file = open(str(index)+'label_e8.npy', 'wb')
...
if __name__ == "__main__":
    ...
    path = '/data/fat/maoyl/ShapeExtraction/'+'12_04'
    # data = np.load('../data/test_data.npy')
    # label = np.load('../data/test_label.npy')
    # path = '/data/fat/maoyl/ShapeExtraction/'+'12_04_test'
    
    np.random.seed(2023)
    
    epsilon = 8
    index = 0
    para = (data, label, index, epsilon)
    perturb(para)
    ...
    # 100 216.87639951705933
    # measuring 753.622878074646
