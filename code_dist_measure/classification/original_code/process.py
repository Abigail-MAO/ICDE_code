...
def process_file(train_data, train_label, times, symbol_size, epsilon, core_index, symbol_distance, test_data, test_label):
    ...
    acc = test.match_clustering(result, test_data, test_label, 4)
    return result, acc
...
def ground_truth():
    print("here")
    # path = '../data/'
    train_data, y_train, X_test, y_test = CachedDatasets().load_dataset("Trace")
    train_data = train_data[y_train < 4]
    train_data = np.load(path+'train_data.npy')
    train_data = train_data.reshape(train_data.shape[0], train_data.shape[1])
    train_label = np.load(path+'train_label.npy')
    new_data = None
    for i in range(1, 4):
        class_indicies = np.where(train_label==i)[0][:50]
        new_data = train_data[class_indicies] if new_data is None else np.concatenate((new_data, train_data[class_indicies]), axis=0)
    ks = KShape(n_clusters=3, verbose=True)
    print(new_data.shape)
    _ = ks.fit_predict(new_data)
    print("train done")
    centers = []
    for i in range(3):
        centers.append(sg.sax_delete_repeat(sg.sax(ks.cluster_centers_[i], 4, 10)))
    return centers
...
if __name__ == '__main__':
    ...
    centers = ['cdabc', 'cdcabc', 'babcdc']
    # centers = ground_truth()
    # print(centers)
    ...
    for epsilon in epsilons:
        ...
        # for _ in range(1):
        result, acc = process_file(data, label, 1, symbol_size, epsilon, 0, symbol_distance, test_data, test_label)
        ...
        print(dtw_dist, sed_dist, euc_dist, acc)
        print(result)
        # espilon=4
        # ['cdabc', 'dcabc', 'abcdc']
        # 12.06 2.67 4.891377263429666 0.865
        #
