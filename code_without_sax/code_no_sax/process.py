...
def process_file(train_data, train_label, times, symbol_size, epsilon, core_index, symbol_distance, test_data, test_label):
    ...
    while t_count<times:
        try:
            ...
            # print("error")
            continue
    file = open('./result_'+str(symbol_size)+'/'+str(epsilon)+'/acc_'+str(core_index)+'.txt', 'w')
...
if __name__ == '__main__':
    ...
    for epsilon in epsilons:
        print("epsilon =", epsilon, "symbol size", symbol_size)
        folder = os.path.exists('./result_'+str(symbol_size)+'/'+str(epsilon))
        if not folder:
            os.makedirs('./result_'+str(symbol_size)+'/'+str(epsilon))
        # else:
        ...
        data_amount = 40000
        path = './data/'
        data = read_data(path+'/'+str(symbol_size)+'_'+str(window_length)+'_'+'data.txt')
        label = read_data(path+'/'+str(symbol_size)+'_'+str(window_length)+'_'+'label.txt', label=True)
        test_data = read_data(path+'/'+str(symbol_size)+'_'+str(window_length)+'_'+'test_data.txt')
        test_label = read_data(path+'/'+str(symbol_size)+'_'+str(window_length)+'_'+'test_label.txt', label=True)
        ...
        files = os.listdir('./result_'+str(symbol_size)+'/'+str(epsilon))
        for path in files:
            file = open('./result_'+str(symbol_size)+'/'+str(epsilon)+'/'+path, 'r')
            sum_accuracy += float(file.readline())
            file.close()
        print("average accuracy:", sum_accuracy/len(files)/times)
        acc_result.append(sum_accuracy/len(files)/times)
    print(acc_result)