...
def sax_generate(size, length, data_amount):
    ...
    saxs = [sg.sax(elem, symbol_size, paa_length) for elem in data]
    data = []
    for i in range(len(saxs)):
        s = ''
        for j in range(len(saxs[i])):
            s += saxs[i][j]
        data.append(s)
    ...
    test_saxs = [sg.sax(elem, symbol_size, paa_length) for elem in data_test]
    data_test = []
    for i in range(len(test_saxs)):
        s = ''
        for j in range(len(test_saxs[i])):
            s += test_saxs[i][j]
        data_test.append(s)
    # random.shuffle(data)
