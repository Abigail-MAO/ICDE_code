...
def perturb(para):
        data, label, index, length, epsilon = para[0], para[1], para[2], para[3], para[4]
        perturbed_data = []
        perturbed_label = []
        np.random.seed(index)
        
        p = np.exp(epsilon/3)/(np.exp(epsilon/3)+1)
        q = 1/(np.exp(epsilon/3)+1)
        for i in range(len(data)):
            ...
            probs = [q, q]
            probs[label[i]] = p
            sample = np.random.choice([0, 1], p=probs)
        ...
        file = open(path+'/patternLDP/'+str(length)+'/'+str(index)+'_data.npy', 'wb')
        np.save(file, perturbed_data)
        file.close()
        perturbed_label = np.array(perturbed_label)
        file = open(path+'/patternLDP/'+str(length)+'/'+str(index)+'_label.npy', 'wb')
...
if __name__ == "__main__":
    ...
    for l in length:
        ...
        folder = os.path.exists(path+'/patternLDP/'+str(l))
        if not folder:
            os.makedirs(path+'/patternLDP/'+str(l))
        data = np.load('data_path'+str(l)+'.npy')
        label = np.load('data_path'+str(l)+'.npy')
        ...
        for index in range(500):
            para.append((data, label, index, l, epsilon))
