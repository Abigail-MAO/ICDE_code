...
def process_file(train_data, train_label, times, symbol_size, epsilon, core_index, symbol_distance, test_data, test_label):
    ...
    while t_count<times:
        try:
            ...
            result = label_match_with_noise(root, symbol_size, levels, data, label, epsilon, clus_num=2)
            # print(result)
            # exit()
            ...
            continue
    file = open('./result/'+str(length)+'/acc_'+str(core_index)+'.txt', 'w')
...
if __name__ == '__main__':
    ...
    for length in lengths:
        print("length =", length)
        folder = os.path.exists('./result/'+str(length))
        if not folder:
            os.makedirs('./result/'+str(length))
        
        path = './data/'+str(length)
        ...
        # for i in range(core_number):
        #     file = open('./result/'+str(length)+'/acc_'+str(i)+'.txt', 'r')
        #     sum_accuracy += float(file.readline())
        #     file.close()
        # print("average accuracy:", sum_accuracy/core_number/times)
        files = os.listdir('./result/'+str(length))
        for path in files:
            file = open('./result/'+str(length)+'/'+path, 'r')
            sum_accuracy += float(file.readline())
            file.close()
