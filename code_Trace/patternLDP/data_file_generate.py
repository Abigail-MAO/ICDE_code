...
if __name__ == '__main__':
    ...
    data = np.load('../data/generate_data_80000.npy')
    label = np.load('../data/generate_label_80000.npy')
    test_data_amount = 1000
    data, data_test, label, label_test = train_test_split(data, label, test_size=test_data_amount, random_state=2023)
    file = open('../data/test_data.npy', 'wb')
    np.save(file, data_test)
    file.close()
    file = open('../data/test_label.npy', 'wb')
    np.save(file, label_test)
    file.close()
    file = open('../data/train_data.npy', 'wb')
    np.save(file, data)
    file.close()
    file = open('../data/train_label.npy', 'wb')
    np.save(file, label)
    file.close()
