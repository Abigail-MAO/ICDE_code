...
def generate_tree(symbol_dist, knowledge, data, label, epsilon, threshold):
    ...
    for level in range(max_level+1):
        ...
        if level == 0:
            ...
            selected_data, data, label = select_data(data, label, math.floor(data_length*0.7/(max_level+1)))
            # print_tree(root)
            ...
            for previous_node in levels_node[level-1]:
                ...
                for elem, count in candidates:
                    ...
                    if elem[0] == previous_node.value:
                        ...
                        levels_node[level].append(node1)
            # print(data_length, max_level, math.floor(data_length/max_level))
            selected_data, data, label = select_data(data, label, math.floor(data_length*0.7/(max_level+1)))
...
def label_match_with_noise(root, symbol_size, levels, data, label, epsilon, clus_num):
    ...
    for index in range(len(data)):
        ...
        for candidate in candidates:
            distance = sd.similar_match(symbol_dist, data[index][:len(candidate)], candidate)
...
def process_file(train_data, train_label, times, symbol_size, epsilon, core_index, symbol_distance, test_data, test_label, window_length):
    sum_acc = 0
    # c_f = open('./result_centroids/'+str(symbol_size)+'_'+str(window_length)+'/'+str(core_index)+'_centroids.txt', 'w+')
    t_count = 0
    ...
    while t_count<times:
        try:
            ...
            continue
    file = open('./result/'+str(symbol_size)+'_'+str(window_length)+'/acc_'+str(core_index)+'.txt', 'w')
...
if __name__ == '__main__':
    # paras = [(4, 5), (4, 10), (4, 15), (4, 20), (4, 25), (3, 10), (5, 10), (6, 10), (7, 10), (8, 10), (9, 10), (10, 10)]
    paras = [(4, 30), (4, 35), (4, 40)]
    for para in paras:
        core_number = 28
        symbol_size = para[0]
        window_length = para[1]
        
        symbol_distance = sd.get_distance(symbol_size)
        epsilons = [4]
        # epsilons = [0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8]
        for epsilon in epsilons:
            print(symbol_size, window_length)
            folder = os.path.exists('./result/'+str(symbol_size)+'_'+str(window_length))
            if not folder:
                os.makedirs('./result/'+str(symbol_size)+'_'+str(window_length))
            else:
                files = os.listdir('./result/'+str(symbol_size)+'_'+str(window_length))
                for path in files:
                    os.remove('./result/'+str(symbol_size)+'_'+str(window_length)+'/'+path)
            ...
            data_amount = 40000
            path = './data/'
            data = read_data(path+'/'+str(symbol_size)+'_'+str(window_length)+'_data.txt')
            label = read_data(path+'/'+str(symbol_size)+'_'+str(window_length)+'_label.txt', label=True)
            test_data = read_data(path+'/'+str(symbol_size)+'_'+str(window_length)+'_test_data.txt')
            test_label = read_data(path+'/'+str(symbol_size)+'_'+str(window_length)+'_test_label.txt', label=True)
            ...
            # process_file(data, label, times, symbol_size, epsilon, -1, symbol_distance, test_data, test_label, window_length)
            pool = mp.Pool(core_number)
            for i in range(core_number):
                pool.apply_async(process_file, args=(data, label, times, symbol_size, epsilon, i, symbol_distance, test_data, test_label, window_length,))
            pool.close()
            pool.join()
