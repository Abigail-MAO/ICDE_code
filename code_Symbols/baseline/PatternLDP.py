...
def pattern_ldp(data, delta, theta, mu, epsilon, k_p, k_i, k_d, pi):
        time = [index for index in range(len(data))]
        ...
        for index in range(len(sample_points)):
            ...
            # if index>1:
                ...
                epsilon_remaind, epsilon_now, b = iar.range(index, alpha, theta, mu, score_array, epsilon_remaind)
        ...
        for index in range(len(sample_points)):
                # print(score_array)
            alpha, perturb_result, epsilon_remaind = iar.perturb(data, w, index, b, epsilon_allocation[index], epsilon, epsilon_remaind, alpha, sample_points)
...
def perturb(para):
        ...
        file = open(path+'/patternLDP/'+process_type+str(epsilon)+'/'+str(index)+'_data.npy', 'wb')
        np.save(file, perturbed_data)
        file.close()
        perturbed_label = np.array(perturbed_label)
        file = open(path+'/patternLDP/'+process_type+str(epsilon)+'/'+str(index)+'_label.npy', 'wb')
...
if __name__ == "__main__":
    ...
    path = '/data/fat/maoyl/ShapeExtraction/'+'07_02'
    ...
    for epsilon in epsilons:
        ...
        folder = os.path.exists(path+'/patternLDP/'+process_type+str(epsilon))
        if not folder:
            os.makedirs(path+'/patternLDP/'+process_type+str(epsilon))
        else:
            files = os.listdir(path+'/patternLDP/'+process_type+str(epsilon))
            for f_path in files:
                os.remove(path+'/patternLDP/'+process_type+str(epsilon)+'/'+f_path)
