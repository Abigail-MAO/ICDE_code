    ...
    for elem in data:
        ...
        for elem_ in elems_agg:
            if elem_ not in count[index]:
                ...
                count[index][elem_] += 1
    pattern_time = time.time()-pattern_time
    for elem in count:
        count[elem] = sorted(count[elem].items(), key=lambda x: x[1], reverse=True)
    pro_time = time.time()-pro_time
    pro_time = pro_time-length_time-pattern_time+length_time/len(selected_data)+pattern_time/len(data)
...
def generate_tree(symbol_dist, knowledge, data, label, epsilon, threshold):
    ...
    for level in range(max_level+1):
        ...
        if level == 0:
            ...
            selected_data, data, label = select_data(data, label, int(data_length/10))
            # print_tree(root)
            ...
            for previous_node in levels_node[level-1]:
                ...
                for elem, count in candidates:
                    ...
                    if elem[0] == previous_node.value:
                        ...
                        levels_node[level].append(node1)
            selected_data, data, label = select_data(data, label, math.floor(data_length/10))
    ...
    pro_time = time.time()-pro_time
    print("tree generate", pro_time-level1_time-level2_time, level1_time, level2_time)
    pro_time = pro_time-level1_time-level2_time+level1_time/level1_users+level2_time/level2_users
    return root, levels_node, data, label, pro_time
...
def label_match_with_noise(root, symbol_size, levels, data, label, epsilon, clus_num):
    ...
    # print(real_count)
    pro_time = time.time()-pro_time
    pro_time = pro_time-user_time+user_time/len(data)
...
def process_file(train_data, train_label, times, symbol_size, epsilon, core_index, symbol_distance, test_data, test_label):
    ...
    knowledge_noised, data, label, time_know = count_position_frequent_pattern_with_noise(data, label, epsilon, symbol_size)
    ...
    result, time_matching = label_match_with_noise(root, symbol_size, levels, data, label, epsilon, clus_num=3)
    # c_f.write(str(result)+'\n')
    # print("label match done!")
    acc, time_testing = test.match_clustering(result, test_data, test_label, symbol_size)
    # print(result, acc)
    sum_acc += acc
    # print("test done!")
    t_count += 1
    
    consum_time = time_know+time_gt+time_matching+time_testing
    print(time_know, time_gt, time_matching, time_testing, consum_time)
    return consum_time
...
if __name__ == '__main__':
    ...
    # data_file generate 12.450032711029053
    # time 16.25497126957727