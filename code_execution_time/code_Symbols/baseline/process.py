...
def count_position_frequent_pattern_with_noise(data, label, epsilon, size, portion=0.1):
    ...
    result, user_end_time = position_count_with_noise(selected_data, epsilon, size)
    ...
    return result, data, label, user_end_time, len(selected_data)
    ...
    for elem in data:
        ...
        for elem_ in elems_agg:
            if elem_ not in count[index]:
                ...
                count[index][elem_] += 1
    # user_end_time = time.time()-user_before_time
    for elem in count:
        count[elem] = sorted(count[elem].items(), key=lambda x: x[1], reverse=True)
    # print(count)
    # return count, user_end_time
    return count
...
def generate_tree(symbol_dist, knowledge, data, label, epsilon, threshold):
    ...
    for level in range(max_level+1):
        ...
        if level == 0:
            ...
            # print_tree(root)
            before_user_time = time.time()
            for seq in selected_data:
                root = count_update_similar_match_with_noise(symbol_dist, seq, root, level, epsilon)
            end_user_time = time.time()-before_user_time
            all_time += end_user_time
            user_amount += len(selected_data)
            # prune
            for node in levels_node[level]:
            ...
            level_before_user_time = time.time()
            for seq in selected_data:
                root = count_update_similar_match_with_noise(symbol_dist, seq, root, level, epsilon)
            level_end_user_time = time.time()-level_before_user_time
            all_time += level_end_user_time
            user_amount += len(selected_data)
            # node prune
    ...
    # print(all_time)
    return root, levels_node, data, label, all_time, user_amount
...
def label_match_with_noise(root, symbol_size, levels, data, epsilon, clus_num):
    ...
    # print(result)
    # sys.exit()
    return result, user_end_time, len(data)
    # return result
...
def process_file(train_data, train_label, times, symbol_size, epsilon, core_index, symbol_distance, test_data, test_label):
    ...
    knowledge_noised, data, label, user_end_time_length, user_for_length = count_position_frequent_pattern_with_noise(data, label, epsilon, symbol_size)
    ...
    if epsilon == 0.5:
        ...
        threshold = 100
    root, levels, data, label, all_time, user_for_tree = generate_tree(symbol_distance, knowledge_noised, data, label, epsilon, threshold)
    # print_tree(root)
    # print(knowledge_noised)
    # print(levels[4])
    # print("tree generate done!")
    result, user_end_time_label, user_for_matching = label_match_with_noise(root, symbol_size, levels, data, epsilon, clus_num=6)
    # c_f.write(str(result)+'\n')
    # print("label match done!")
    acc, ari, ri, test_time = test.match_clustering(result, test_data, test_label, symbol_size)

    consume_time += (time.time()-pro_start_time-user_end_time_length-all_time-user_end_time_label-test_time)+(user_end_time_length/user_for_length+all_time/user_for_tree+user_end_time_label/user_for_matching+test_time/len(test_data))
    # print(consume_time)
...
if __name__ == '__main__':
    ...
    for epsilon in epsilons:
        ...
        files = os.listdir('./result_02/'+str(epsilon))
        for path in files:
            file = open('./result_02/'+str(epsilon)+'/'+path, 'r')
            sum_accuracy += float(file.readline())
            file.close()
