...
# path = '/data/fat/maoyl/ShapeExtraction/Symbols_02_28/patternLDP/'
...
def perturb(para):
        ...
        file = open(path+str(epsilon)+'/'+str(index)+'_data.npy', 'wb')
        np.save(file, perturbed_data)
        file.close()
        # perturbed_label = np.array(perturbed_label)
        # file = open(path+'/'+str(epsilon)+'/'+str(index)+'_label.npy', 'wb')
...
if __name__ == "__main__":
    ...
    for epsilon in epsilons:
        ...
        print(path)
        folder = os.path.exists(path+str(epsilon))
        if not folder:
            os.makedirs(path+str(epsilon))
        # else:
